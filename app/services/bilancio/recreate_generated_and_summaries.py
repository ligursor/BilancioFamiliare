"""Reusable logic to recreate generated transactions from recurring and regenerate monthly summaries.
...
def recreate_generated_and_summaries(months=6, base_date=None, initial_year=None, initial_month=None, initial_saldo=None, full_wipe=False):
    ...
    Parameters:
      ...
      - full_wipe: if True, DELETE all rows from `transazione` and `monthly_summary` before repopulating.
    ...
    if full_wipe:
        try:
            tx_count = db.session.execute(text('SELECT COUNT(*) FROM transazione')).fetchone()[0]
        except Exception:
            tx_count = None
        try:
            ms_count = db.session.execute(text('SELECT COUNT(*) FROM monthly_summary')).fetchone()[0]
        except Exception:
            ms_count = None
        try:
            db.session.execute(text('DELETE FROM transazione'))
            db.session.commit()
        ...
        try:
            db.session.execute(text('DELETE FROM monthly_summary'))
            db.session.commit()
