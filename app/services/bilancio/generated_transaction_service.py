...
class GeneratedTransactionService(BaseService):
    """Servizio che popola la tabella `transazione` a partire dalle ricorrenze.

    Nota: anche se il nome del servizio è rimasto per compatibilità, ora questo inserisce
    direttamente record nella tabella `transazione`. Per tracciare le transazioni generate
    dalla ricorrenza, viene creato (se non esiste) un record madre in `transazione` con
    ...
    def populate_horizon_from_recurring(self, months=6, base_date=None):
        ...
        try:
            ...
            rows = db.session.execute(
                text("SELECT id, descrizione, importo, categoria_id, tipo, giorno, attivo, cadenza, prossima_data, skip_month_if_annual FROM recurring_transaction WHERE attivo=1")
            ).fetchall()
        ...
        for r in recs:
            # Non creiamo più una "transazione madre" — le istanze generate
            # sono scritte direttamente nella tabella `transazione` con
            # `id_recurring_tx` che punta alla r.id (recurring_transaction.id).
            ...
            for i in range(months):
                ...
                exists = Transazione.query.filter_by(id_recurring_tx=getattr(r, 'id', None), data=candidate_date).first()
                if exists:
                    continue

                # crea la transazione programmata (data_effettiva None -> in attesa)
                tx = Transazione(
                    data=candidate_date,
                    data_effettiva=None,
