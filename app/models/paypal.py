...
class PaypalPiano(db.Model):
    ...
    def __repr__(self):
        return f'<PaypalPiano {self.descrizione}: {self.importo_totale}>'

class PaypalRata(db.Model):
    """Modello per le singole rate dei piani PayPal"""
    __tablename__ = 'paypal_rata'
    ...
    transazione_id = db.Column(db.Integer, db.ForeignKey('transazione.id'), nullable=True)
    transazione = db.relationship('Transazione', backref=db.backref('rata_paypal', uselist=False))
    
    def __repr__(self):
        return f'<PaypalRata {self.piano.descrizione} - Rata {self.numero_rata}: {self.importo}>'
