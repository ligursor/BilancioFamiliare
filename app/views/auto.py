"""
Blueprint per la gestione del garage auto
...
@auto_bp.route('/')
def garage():
    """Dashboard del garage auto - replica dell'implementazione originale"""
    try:
...
def dettaglio(veicolo_id):
    ...
    try:
        ...
        # Calcolo totali per il box informazioni
...
def rimuovi_veicolo(veicolo_id):
    """Rimuove un veicolo dal garage (soft delete: elimina solo dalla vista, mantiene i dati storici)"""
    try:
        veicolo = Veicolo.query.get_or_404(veicolo_id)
        # Soft delete: imposta un flag oppure elimina dalla tabella principale
...
def modifica_veicolo(veicolo_id):
    """Modifica i dati di un veicolo esistente"""
    try:
        veicolo = Veicolo.query.get_or_404(veicolo_id)
        # Conversione del mese di scadenza bollo
        mese_scadenza = request.form.get('mese_scadenza_bollo')
        ...
        veicolo.costo_finanziamento = float(request.form['costo_finanziamento']) if request.form.get('costo_finanziamento') else None
        veicolo.numero_rate = int(request.form['numero_rate']) if request.form.get('numero_rate') else None
        veicolo.rata_mensile = float(request.form['rata_mensile']) if request.form.get('rata_mensile') else None
        db.session.commit()
